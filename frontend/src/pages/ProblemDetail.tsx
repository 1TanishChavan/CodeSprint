...
const ProblemDetail: React.FC<ProblemDetailProps> = ({ user }) => {
  ...
  const [latestSubmission, setLatestSubmission] = useState<{ status: string, results: SubmissionResult[] } | null>(null);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <p className={`mb-4 text-lg font-semibold ${getDifficultyColor(problem.difficulty)}`}>
        Difficulty: {problem.difficulty}
      </p>
      <div className="mb-6 prose dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: problem.description }} />
      <h2 className="text-2xl font-semibold mb-2">Sample Test Cases</h2>
      <div className="mb-6">
        {problem.publicTestCases.map((testCase, index) => (
          <div key={index} className="mb-2 p-4 bg-gray-100 dark:bg-gray-700 rounded">
            <p className="font-mono">Input: {testCase.input}</p>
            <p className="font-mono">Output: {testCase.output}</p>
          </div>
        ))}
      </div>
      {/* <CodingEnvironment problemId={problem.id} onSubmissionComplete={handleSubmissionComplete} /> */}
      <CodingEnvironment problemId={problem.id} onSubmissionComplete={handleSubmissionComplete} user={user} />
      {latestSubmission && (
        <div className="mt-8">
          <h2 className="text-2xl font-semibold mb-4">Latest Submission Results</h2>
          <p className={`text-lg font-semibold ${latestSubmission.status === 'Accepted' ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <ul className="mt-4 space-y-4">
            {latestSubmission.results.map((result, index) => (
              <li key={index} className="bg-gray-100 dark:bg-gray-700 p-4 rounded">
                ...
                <p className="font-mono">Expected Output: {result.expectedOutput}</p>
                <p className="font-mono">Actual Output: {result.actualOutput}</p>
                <p className={result.matches ? 'text-green-500' : 'text-red-500'}>
